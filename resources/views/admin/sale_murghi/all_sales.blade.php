						...
						<tbody>
                            @foreach($sales AS $sale)
                                <tr style="border-color:black;">
                                    ...
                                    {{-- <td>{{ @implode(",", $sale->outwardDetail->pluck('item')->pluck('name')->toArray()) }}</td> --}}
                                    {{-- <td>{{ @implode(",", $sale->outwardDetail->pluck('quantity')->toArray()) }}</td> --}}
                                    ...
                                    <td width="120">
                                        <a href="{{route('admin.sales.edit_sale', $sale->hashid)}}" class="btn btn-primary btn-xs waves-effect waves-light">
                                            <i class="fas fa-edit"></i>
                                        </a>
                                        <button type="button" onclick="ajaxRequest(this)" data-url="{{ route('admin.sales.delete_sale', ['id'=>$sale->hashid]) }}"  class="btn btn-danger btn-xs waves-effect waves-light">
